...
class InventoryOptimizationEngine:
    ...
    def prepare_demand_data(self):
        ...
        daily_demand = self.transactions.groupby(['product_id', 'transaction_date']).agg({
            'quantity': 'sum',
            'total_amount': 'sum',
        ...
        complete_demand = complete_data.merge(daily_demand, on=['product_id', 'transaction_date'], how='left')
        complete_demand[['quantity', 'total_amount', 'transaction_count']] = complete_demand[['quantity', 'total_amount', 'transaction_count']].fillna(0)
        
        # Fill product information
        product_info = self.products.set_index('product_id')
        for col in ['product_name', 'category', 'subcategory', 'brand', 'unit_price', 'supplier_id']:
        ...
        complete_demand['day_of_week'] = complete_demand['transaction_date'].dt.dayofweek
        complete_demand['day_of_month'] = complete_demand['transaction_date'].dt.day
        complete_demand['is_weekend'] = complete_demand['day_of_week'].isin([5, 6]).astype(int)
        complete_demand['is_month_start'] = (complete_demand['day_of_month'] <= 7).astype(int)
        complete_demand['is_month_end'] = (complete_demand['day_of_month'] >= 24).astype(int)
        ...
        complete_demand = complete_demand.sort_values(['product_id', 'transaction_date'])
        
        for window in [7, 14, 30]:
            complete_demand[f'quantity_rolling_{window}d'] = complete_demand.groupby('product_id')['quantity'].transform(
                lambda x: x.rolling(window=window, min_periods=1).mean()
            )
            complete_demand[f'quantity_trend_{window}d'] = complete_demand.groupby('product_id')[f'quantity_rolling_{window}d'].transform(
                lambda x: x.pct_change(periods=window).fillna(0)
            )
        
        # Calculate demand volatility
        complete_demand['demand_volatility'] = complete_demand.groupby('product_id')['quantity'].transform(
            lambda x: x.rolling(window=30, min_periods=1).std().fillna(0)
        )
        
        # Product lifecycle features
        product_stats = complete_demand.groupby('product_id').agg({
            'quantity': ['sum', 'mean', 'std', 'max'],
            'transaction_date': ['min', 'max']
        }).round(2)
        
        product_stats.columns = ['total_demand', 'avg_daily_demand', 'demand_std', 'max_daily_demand', 'first_sale_date', 'last_sale_date']
        product_stats = product_stats.reset_index()
        
        # Calculate product age and lifecycle stage
        reference_date = complete_demand['transaction_date'].max()
        product_stats['product_age_days'] = (reference_date - product_stats['first_sale_date']).dt.days
        product_stats['days_since_last_sale'] = (reference_date - product_stats['last_sale_date']).dt.days
        ...
        complete_demand = complete_demand.merge(product_stats[['product_id', 'lifecycle_stage', 'avg_daily_demand']], on='product_id', how='left')
        
        self.demand_data = complete_demand
        
        print(f"✅ Demand data prepared for {len(products)} products over {len(date_range)} days")
    ...
    def train_demand_forecasting_models(self, forecast_horizon=30):
        ...
        feature_columns = [
            'month', 'day_of_week', 'day_of_month', 'is_weekend', 'is_month_start', 'is_month_end',
            'quantity_rolling_7d', 'quantity_rolling_14d', 'quantity_rolling_30d',
            'quantity_trend_7d', 'quantity_trend_14d', 'quantity_trend_30d',
            'demand_volatility', 'unit_price'
        ]
        
        # Encode categorical variables
        category_dummies = pd.get_dummies(self.demand_data['category'], prefix='category')
        season_dummies = pd.get_dummies(self.demand_data['season'], prefix='season')
        lifecycle_dummies = pd.get_dummies(self.demand_data['lifecycle_stage'], prefix='lifecycle')
            ...
            for forecast_date in forecast_dates:
                ...
                for col in self.feature_columns:
                    if col.startswith(('category_', 'season_', 'lifecycle_')):
                        if col in [f"category_{product_row['category']}", 
                ...
                feature_vector = pd.DataFrame([forecast_features])[self.feature_columns].fillna(0)
                
                # Make prediction
                predicted_demand = max(0, self.forecast_models['demand'].predict(feature_vector)[0])
    ...
    def optimize_inventory_levels(self, service_level=0.95, lead_time_days=7):
        ...
        for product_id in forecasts['product_id'].unique():
            ...
            product_info = self.demand_data[self.demand_data['product_id'] == product_id].iloc[-1]
            ...
            if annual_demand > 0 and product_info['unit_price'] > 0:
                eoq = np.sqrt((2 * annual_demand * ordering_cost) / (holding_cost_rate * product_info['unit_price']))
            else:
            ...
            # Risk assessment
            stockout_risk = 'High' if demand_std > avg_daily_demand else 'Medium' if demand_std > avg_daily_demand * 0.5 else 'Low'
        ...
        print(f"✅ Inventory optimization completed for {len(inventory_optimization)} products")
    ...
    def generate_procurement_recommendations(self):
        ...
        high_priority = self.inventory_recommendations[
            ...
            (self.inventory_recommendations['avg_daily_demand'] > self.inventory_recommendations['avg_daily_demand'].quantile(0.8))
        ...
        medium_priority = self.inventory_recommendations[
            ...
            (self.inventory_recommendations['avg_daily_demand'] > self.inventory_recommendations['avg_daily_demand'].quantile(0.5))
        ].sort_values('avg_daily_demand', ascending=False)
        ...
        recommendations = {
            'high_priority_reorders': {
                ...
                'total_value': (high_priority['economic_order_quantity'] * high_priority['unit_price']).sum()
            },
            'medium_priority_reorders': {
                'count': len(medium_priority),
                'products': medium_priority.head(20).to_dict('records'),
                'total_value': (medium_priority['economic_order_quantity'] * medium_priority['unit_price']).sum()
            },
            'overstock_alerts': {
                'count': len(overstock_alerts),
                'products': overstock_alerts.head(10).to_dict('records'),
                'total_value': (overstock_alerts['economic_order_quantity'] * overstock_alerts['unit_price']).sum()
            }
        }
        
        # Calculate business impact
        total_products = len(self.inventory_recommendations)
        avg_inventory_turnover = self.inventory_recommendations['inventory_turnover'].mean()
        
        print(f"\n📊 Procurement Recommendations Summary:")
        print(f"   High Priority Reorders: {recommendations['high_priority_reorders']['count']} products")
        print(f"   Medium Priority Reorders: {recommendations['medium_priority_reorders']['count']} products")
        print(f"   Overstock Alerts: {recommendations['overstock_alerts']['count']} products")
...
def main():
    ...
    procurement_recommendations = inventory_engine.generate_procurement_recommendations()
