...
class ChurnPredictionEngine:
    ...
    def prepare_churn_features(self, churn_threshold_days=90):
        ...
        customer_stats = self.transactions.groupby('customer_id').agg({
            ...
            'store_id': [
                ('unique_stores_visited', 'nunique')
            ],
            'channel': [
                ('unique_channels_used', 'nunique')
            ],
            'discount_applied': [
        ...
        customer_stats.columns = [col[1] if col[1] else col[0] for col in customer_stats.columns]
        customer_stats = customer_stats.reset_index()
        
        # Calculate time-based features
        customer_stats['days_since_last_purchase'] = (reference_date - customer_stats['last_purchase_date']).dt.days
        customer_stats['customer_lifetime_days'] = (customer_stats['last_purchase_date'] - customer_stats['first_purchase_date']).dt.days
        customer_stats['purchase_frequency_per_month'] = customer_stats['total_transactions'] / (customer_stats['customer_lifetime_days'] / 30 + 1)
        
        # Behavioral features
        customer_stats['avg_days_between_purchases'] = customer_stats['customer_lifetime_days'] / (customer_stats['total_transactions'] + 1)
        customer_stats['spending_acceleration'] = customer_stats['total_spent'] / (customer_stats['customer_lifetime_days'] + 1)
        customer_stats['transaction_value_consistency'] = customer_stats['std_transaction_value'] / (customer_stats['avg_transaction_value'] + 1)
        ...
        recent_stats = recent_transactions.groupby('customer_id').agg({
            'total_amount': ['sum', 'count'],
            'quantity': 'sum'
        }).round(2)
        
        recent_stats.columns = ['recent_30d_spent', 'recent_30d_transactions', 'recent_30d_items']
        ...
        customer_stats[['recent_30d_spent', 'recent_30d_transactions', 'recent_30d_items']] = customer_stats[['recent_30d_spent', 'recent_30d_transactions', 'recent_30d_items']].fillna(0)
        
        # Add customer demographics
        customer_stats = customer_stats.merge(self.customers, on='customer_id', how='left')
        
        # Calculate customer tenure
        customer_stats['customer_tenure_days'] = (reference_date - customer_stats['member_since']).dt.days
        
        # Create churn label
        customer_stats['is_churned'] = (customer_stats['days_since_last_purchase'] > churn_threshold_days).astype(int)
        
        # Advanced behavioral indicators
        customer_stats['declining_spend_trend'] = (customer_stats['recent_30d_spent'] < customer_stats['avg_transaction_value']).astype(int)
        customer_stats['low_engagement_score'] = ((customer_stats['recent_30d_transactions'] == 0) & 
                                                 (customer_stats['days_since_last_purchase'] > 30)).astype(int)
        
        # Loyalty and engagement features
        le = LabelEncoder()
        customer_stats['loyalty_tier_encoded'] = le.fit_transform(customer_stats['loyalty_tier'])
        ...
        customer_stats['churn_risk_score'] = (
            ...
            (1 - customer_stats['recent_30d_transactions'] / customer_stats['purchase_frequency_per_month'].clip(lower=1)) * 0.2 +
    ...
    def train_churn_model(self, test_size=0.2, hyperparameter_tuning=True):
        ...
        feature_columns = [
            'total_transactions', 'total_spent', 'avg_transaction_value', 'std_transaction_value',
            'total_items_purchased', 'avg_items_per_transaction', 'unique_stores_visited',
            'unique_channels_used', 'discount_usage_rate', 'total_discounts_received',
            'days_since_last_purchase', 'customer_lifetime_days', 'purchase_frequency_per_month',
            'avg_days_between_purchases', 'spending_acceleration', 'transaction_value_consistency',
            'recent_30d_spent', 'recent_30d_transactions', 'recent_30d_items',
            'age', 'household_size', 'estimated_income', 'customer_tenure_days',
            'loyalty_tier_encoded', 'gender_encoded', 'declining_spend_trend',
            'low_engagement_score', 'churn_risk_score'
        ]
    ...
    def _get_tuned_models(self, X_train, y_train):
        ...
        lr_grid = GridSearchCV(
            ...
            lr_params, cv=3, scoring='roc_auc', n_jobs=-1
        )
    ...
    def predict_churn_risk(self, customer_ids=None):
        ...
        if customer_ids is not None:
            data = self.churn_features[self.churn_features['customer_id'].isin(customer_ids)]
        else:
    ...
    def generate_retention_strategies(self):
        ...
        strategies = {
            'High Risk': {
                'customers': len(churn_predictions[churn_predictions['risk_category'] == 'High Risk']),
                'strategies': [
            ...
            'Medium Risk': {
                'customers': len(churn_predictions[churn_predictions['risk_category'] == 'Medium Risk']),
                'strategies': [
            ...
            'Low Risk': {
                'customers': len(churn_predictions[churn_predictions['risk_category'] == 'Low Risk']),
                'strategies': [
        ...
        total_at_risk_customers = strategies['High Risk']['customers'] + strategies['Medium Risk']['customers']
        total_campaign_cost = (strategies['High Risk']['customers'] * strategies['High Risk']['campaign_cost_per_customer'] +
                              strategies['Medium Risk']['customers'] * strategies['Medium Risk']['campaign_cost_per_customer'] +
                              strategies['Low Risk']['customers'] * strategies['Low Risk']['campaign_cost_per_customer'])
        
        # Estimate average customer value
        avg_customer_value = self.churn_features['total_spent'].mean()
        
        expected_retained_customers = (
            strategies['High Risk']['customers'] * strategies['High Risk']['expected_retention_rate'] +
            strategies['Medium Risk']['customers'] * strategies['Medium Risk']['expected_retention_rate'] +
            strategies['Low Risk']['customers'] * strategies['Low Risk']['expected_retention_rate']
        )
    ...
    def create_churn_dashboard_data(self):
        ...
        dashboard_data = {
            'churn_summary': {
                ...
                'churned_customers': len(churn_predictions[churn_predictions['is_churned'] == 1]),
                'churn_rate': churn_predictions['is_churned'].mean(),
                'high_risk_customers': len(churn_predictions[churn_predictions['risk_category'] == 'High Risk']),
                'medium_risk_customers': len(churn_predictions[churn_predictions['risk_category'] == 'Medium Risk']),
                'low_risk_customers': len(churn_predictions[churn_predictions['risk_category'] == 'Low Risk'])
            },
            'risk_distribution': churn_predictions['risk_category'].value_counts().to_dict(),
            'high_risk_customers': churn_predictions[
                churn_predictions['risk_category'] == 'High Risk'
            ][['customer_id', 'churn_probability']].head(20).to_dict('records')
        }
