...
class RealTimeAnalyticsEngine:
    ...
    def simulate_transaction_stream(self, base_data_path, duration_minutes=5, transactions_per_minute=10):
        ...
        print(f"✅ Stream simulation completed: {transaction_count} transactions processed")
    ...
    def calculate_real_time_kpis(self):
        ...
        kpis = {
            ...
            'total_revenue': recent_df['total_amount'].sum() if 'total_amount' in recent_df.columns else 0,
            'avg_order_value': recent_df['total_amount'].mean() if 'total_amount' in recent_df.columns else 0,
            'unique_customers': recent_df['customer_id'].nunique() if 'customer_id' in recent_df.columns else 0,
            'transactions_per_minute': len(recent_df) / 5,  # 5-minute window
            'revenue_per_minute': (recent_df['total_amount'].sum() / 5) if 'total_amount' in recent_df.columns else 0
        }
    ...
    def check_alerts(self, current_kpis):
        ...
        if current_kpis['total_revenue'] > 0 and previous_kpis['total_revenue'] > 0:
            revenue_change = (current_kpis['total_revenue'] - previous_kpis['total_revenue']) / previous_kpis['total_revenue']
        ...
        if previous_kpis['transaction_count'] > 0:
            volume_ratio = current_kpis['transaction_count'] / previous_kpis['transaction_count']
        ...
        if current_kpis['avg_order_value'] > 0 and previous_kpis['avg_order_value'] > 0:
            aov_change = (current_kpis['avg_order_value'] - previous_kpis['avg_order_value']) / previous_kpis['avg_order_value']
        ...
        if previous_kpis['unique_customers'] > 0:
            customer_change = (current_kpis['unique_customers'] - previous_kpis['unique_customers']) / previous_kpis['unique_customers']
        ...
        while self.is_monitoring:
            try:
                ...
                if event_type == 'new_transaction':
                    ...
                    if new_alerts:
                        recommendations = self.generate_real_time_recommendations(current_kpis, new_alerts)
    ...
    def get_live_dashboard_data(self):
        ...
        dashboard_data = {
            ...
            'kpi_history': list(self.kpi_buffer)[-20:] if len(self.kpi_buffer) > 20 else list(self.kpi_buffer),
            'buffer_status': {
    ...
    def export_real_time_data(self):
        ...
        if self.transaction_buffer:
            ...
            transactions_df.to_csv(f'{self.results_dir}/real_time_transactions_{timestamp}.csv', index=False)
        
        # Export KPIs
        if self.kpi_buffer:
            kpis_df = pd.DataFrame(list(self.kpi_buffer))
            kpis_df.to_csv(f'{self.results_dir}/real_time_kpis_{timestamp}.csv', index=False)
        
        # Export alerts
        if self.alerts:
            alerts_df = pd.DataFrame(self.alerts)
            alerts_df.to_csv(f'{self.results_dir}/real_time_alerts_{timestamp}.csv', index=False)
...
def main():
    ...
    results = rt_engine.run_real_time_demo('data/processed/transactions.csv', duration_minutes=1)
