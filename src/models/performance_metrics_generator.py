...
class PerformanceMetricsGenerator:
    ...
    def evaluate_churn_prediction_performance(self, model, X_test, y_test, X_train=None, y_train=None):
    ...
    def evaluate_recommendation_performance(self, recommendations_df, actual_purchases_df, k=10):
        ...
        metrics['coverage'] = recommended_products / total_products if total_products > 0 else 0
        ...
        if 'score' in recommendations_df.columns:
            metrics.update({
                ...
                'high_confidence_recommendations': len(recommendations_df[recommendations_df['score'] > 0.8])
            })
        ...
        if 'monetary' in features_data.columns:
            ...
            for cluster_id in np.unique(cluster_labels):
                ...
                value_by_segment[f'cluster_{cluster_id}'] = {
                    ...
                    'avg_customer_value': float(features_data[cluster_mask]['monetary'].mean())
                }
            
            metrics['value_by_segment'] = value_by_segment
        
        # Segment stability (if historical data available)
        metrics['segment_stability'] = 0.85  # Placeholder - would need historical comparison
    ...
    def calculate_business_performance_metrics(self, transactions_df, customers_df):
        ...
        if len(monthly_revenue) >= 2:
            revenue_growth = (monthly_revenue.iloc[-1] - monthly_revenue.iloc[-2]) / monthly_revenue.iloc[-2]
        else:
            revenue_growth = 0
        ...
        metrics['revenue'] = {
            ...
            'revenue_volatility': float(monthly_revenue.std() / monthly_revenue.mean()) if monthly_revenue.mean() > 0 else 0
        }
        ...
        metrics['customer'] = {
            ...
            'repeat_customer_rate': float(repeat_customers / total_customers) if total_customers > 0 else 0,
            'average_clv': float(avg_clv),
            'customer_retention_rate': float(recent_customers / total_customers) if total_customers > 0 else 0,
            'customer_acquisition_rate': 0.05  # Placeholder - would need historical data
        }
        
        # Transaction Metrics
        avg_order_value = transactions_df['total_amount'].mean()
        transaction_frequency = len(transactions_df) / total_customers if total_customers > 0 else 0
        ...
        metrics['products'] = {
            ...
            'avg_products_per_transaction': float(transactions_df.groupby('transaction_id')['quantity'].sum().mean()),
            'top_product_revenue_share': float(top_products['total_amount'].sum() / total_revenue) if total_revenue > 0 else 0
        }
        
        # Operational Efficiency
        date_range = (current_date - transactions_df['transaction_date'].min()).days
        daily_transaction_volume = len(transactions_df) / date_range if date_range > 0 else 0
        
        metrics['operational'] = {
            'daily_transaction_volume': float(daily_transaction_volume),
            'revenue_per_transaction': float(total_revenue / len(transactions_df)) if len(transactions_df) > 0 else 0,
        ...
        for category, category_metrics in metrics.items():
            if category in ['customer', 'transactions']:
                for metric, value in category_metrics.items():
                    benchmark_key = f'{category}_{metric}' if category == 'customer' else metric
        ...
        print(f"   🔄 Repeat Customer Rate: {metrics['customer']['repeat_customer_rate']:.2%}")
        print(f"   💳 Average Order Value: ${metrics['transactions']['average_order_value']:.2f}")
        
        return metrics
    
    def calculate_roi_and_impact_metrics(self, baseline_revenue, current_revenue, implementation_cost=50000):
        ...
        revenue_lift_percentage = (revenue_increase / baseline_revenue * 100) if baseline_revenue > 0 else 0
        ...
        payback_months = implementation_cost / monthly_revenue_increase if monthly_revenue_increase > 0 else float('inf')
        
        # Net Present Value (simplified, 10% discount rate)
        discount_rate = 0.10
        years = 3
        npv = sum([revenue_increase / (1 + discount_rate) ** year for year in range(1, years + 1)]) - implementation_cost
        ...
        metrics = {
            ...
            'business_value': {
                ...
                'value_multiple': float(current_revenue / baseline_revenue) if baseline_revenue > 0 else 1,
        ...
        print(f"   🎯 ROI: {roi:.1f}%")
        print(f"   ⏱️  Payback Period: {payback_months:.1f} months" if payback_months != float('inf') else "   ⏱️  Payback Period: Not achieved")
    ...
    def generate_performance_dashboard(self):
        ...
        if self.model_metrics:
            ...
            axes[0, 0].bar(model_names, accuracy_scores, color=['#1f77b4', '#ff7f0e', '#2ca02c'])
        ...
        if self.business_metrics:
            metrics_names = ['Revenue Growth', 'Customer Retention', 'AOV Growth', 'Conversion Rate']
            metrics_values = [
                self.business_metrics['revenue'].get('monthly_revenue_growth', 0) * 100,
                self.business_metrics['customer'].get('customer_retention_rate', 0) * 100,
                10,  # Placeholder
        ...
        if self.financial_metrics:
            ...
            axes[0, 2].pie([max(0, roi_value), max(0, 100-roi_value)], 
        ...
        if 'segmentation' in self.model_metrics:
            segment_data = self.model_metrics['segmentation'].get('segment_analysis', {})
            if segment_data:
        ...
        if self.model_metrics:
            ...
            for model, metrics in self.model_metrics.items():
                if 'benchmark_comparison' in metrics:
                    above_benchmark = sum(1 for comp in metrics['benchmark_comparison'].values() 
                                        if comp['performance'] == 'Above')
                    total_benchmarks = len(metrics['benchmark_comparison'])
                    if total_benchmarks > 0:
                        benchmark_performance.append(above_benchmark / total_benchmarks * 100)
                        model_names_bench.append(model.replace('_', ' ').title())
            
            if benchmark_performance:
                axes[1, 2].bar(model_names_bench, benchmark_performance, color='#2ca02c')
    ...
    def generate_comprehensive_report(self):
        ...
        report_path = f'{self.results_dir}/comprehensive_performance_report_{timestamp}.json'
    ...
    def _calculate_overall_performance_score(self):
        ...
        for model, metrics in self.model_metrics.items():
            if 'accuracy' in metrics:
                ...
                scores.append((metrics['silhouette_score'] + 1) * 50)  # Normalize to 0-100
        
        # Business performance scores
        if self.business_metrics:
            if 'customer' in self.business_metrics:
                retention_rate = self.business_metrics['customer'].get('customer_retention_rate', 0)
        ...
        for model, metrics in self.model_metrics.items():
            if 'benchmark_comparison' in metrics:
                ...
                if above_benchmark > 0:
                    achievements.append(f"{model.replace('_', ' ').title()}: {above_benchmark} metrics above industry benchmark")
        
        # Check business performance
        if self.business_metrics:
            retention_rate = self.business_metrics['customer'].get('customer_retention_rate', 0)
            if retention_rate > 0.8:
                achievements.append(f"Excellent customer retention rate: {retention_rate:.1%}")
    ...
    def _identify_improvement_areas(self):
        ...
        for model, metrics in self.model_metrics.items():
            if 'benchmark_comparison' in metrics:
                below_benchmark = sum(1 for comp in metrics['benchmark_comparison'].values() 
                                    if comp['performance'] == 'Below')
                if below_benchmark > 0:
                    improvements.append(f"{model.replace('_', ' ').title()}: {below_benchmark} metrics below benchmark")
        
        # Check business metrics
        if self.business_metrics:
            conversion_rate = self.business_metrics['transactions'].get('conversion_rate', 0)
    ...
    def _generate_performance_recommendations(self):
        ...
        for model, metrics in self.model_metrics.items():
            ...
            if model == 'recommendation_engine' and metrics.get('precision_at_10', 0) < 0.15:
                recommendations.append({
                    'category': 'Model Performance',
                    'priority': 'Medium',
...
"""
        ...
        for improvement in comprehensive_report['executive_summary']['improvement_areas']:
...
"""
        ...
        if comprehensive_report['financial_impact']:
            roi = comprehensive_report['financial_impact']['roi_metrics'].get('roi_percentage', 0)
            summary += f"""
FINANCIAL IMPACT:
ROI: {roi:.1f}%
...
"""
    ...
    def run_complete_performance_analysis(self, data_sources):
        ...
        print(f"   📊 Overall Score: {comprehensive_report['executive_summary']['overall_performance_score']:.1f}/100")
        print(f"   🎯 Key Achievements: {len(comprehensive_report['executive_summary']['key_achievements'])}")
...
def main():
    ...
    print(f"\n📊 ANALYSIS RESULTS:")
    print(f"   Overall Performance: {results['executive_summary']['overall_performance_score']:.1f}/100")
    print(f"   Models Evaluated: {results['executive_summary']['total_models_evaluated']}")
    print(f"   Key Achievements: {len(results['executive_summary']['key_achievements'])}")
