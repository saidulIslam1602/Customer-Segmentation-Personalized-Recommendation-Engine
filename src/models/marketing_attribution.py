...
class MarketingAttributionEngine:
    ...
    def prepare_attribution_data(self):
        ...
        self.digital_events["event_date"] = pd.to_datetime(
            self.digital_events["event_date"]
        )
        ...
        for customer_id in self.customers["customer_id"].unique():
            ...
            customer_events = self.digital_events[
                self.digital_events["customer_id"] == customer_id
            ...
            attribution_window_start = first_purchase["transaction_date"] - timedelta(
                days=30
            )
                ...
                touchpoints.append(
                    {
                        ...
                        "days_before_purchase": (
                            first_purchase["transaction_date"] - event["event_date"]
                        ).days,
                    }
                )
        ...
        for channel in all_channels:
            attribution_df[f"attribution_{channel}"] = attribution_df[
                "multi_touch_attribution"
            ].apply(lambda x: x.get(channel, 0))

        # Calculate time-based features
        attribution_df["days_since_first_purchase"] = (
            attribution_df["first_purchase_date"].max()
            - attribution_df["first_purchase_date"]
        ).dt.days

        attribution_df["is_recent_acquisition"] = (
            attribution_df["days_since_first_purchase"] <= 30
        ).astype(int)
        attribution_df["acquisition_month"] = attribution_df[
            "first_purchase_date"
        ].dt.month
        attribution_df["acquisition_quarter"] = attribution_df[
            "first_purchase_date"
        ].dt.quarter

        # Customer value segments
        attribution_df["clv_quartile"] = pd.qcut(
            attribution_df["total_customer_value"],
            q=4,
    ...
    def analyze_channel_performance(self):
        ...
        attribution_columns = [
            col
            for col in self.attribution_data.columns
            if col.startswith("attribution_")
        ]
        ...
        for col in attribution_columns:
            ...
            # Revenue attribution
            attributed_revenue = (
                attributed_customers["total_customer_value"] * attributed_customers[col]
            ).sum()
            ...
            premium_customers_pct = (
                attributed_customers["clv_quartile"] == "Premium"
            ).mean()
            ...
            roi = (
                (attributed_revenue / (total_customers * estimated_cac)) - 1
            ...
            # Time to conversion
            avg_days_to_convert = (
                attributed_customers.groupby("customer_id")
                .apply(
                    lambda x: (
                        x["first_purchase_date"].iloc[0]
                        - x["first_purchase_date"].iloc[0]
                    ).days
                )
                .mean()
        ...
        channel_performance_df = channel_performance_df.sort_values(
    ...
    def optimize_budget_allocation(self, total_budget=100000):
        ...
        current_allocation["current_budget_pct"] = (
            current_allocation["attributed_revenue"]
            / current_allocation["attributed_revenue"].sum()
        )
        current_allocation["current_budget"] = (
            current_allocation["current_budget_pct"] * total_budget
        )

        # Optimization based on efficiency and ROI
        # Weight channels by efficiency score and ROI
        current_allocation["efficiency_weight"] = (
            current_allocation["efficiency_score"]
            / current_allocation["efficiency_score"].sum()
        ) * 0.6
        ...
        current_allocation["optimized_budget_pct"] = (
            current_allocation["optimized_weight"]
            / current_allocation["optimized_weight"].sum()
        )
        current_allocation["optimized_budget"] = (
            current_allocation["optimized_budget_pct"] * total_budget
        )
        ...
        # Recommendations
        increase_budget = current_allocation[
            current_allocation["budget_change"] > 1000
        ].sort_values("budget_change", ascending=False)
        decrease_budget = current_allocation[
            current_allocation["budget_change"] < -1000
        ].sort_values("budget_change")
        ...
        optimization_results = {
            ...
            "recommendations": {
                "increase_budget": increase_budget[
                    ["channel", "budget_change", "roi", "efficiency_score"]
                ].to_dict("records"),
            ...
            "expected_improvement": {
                "roi_improvement": (
                    current_allocation["roi"]
                    * current_allocation["optimized_budget_pct"]
                ).sum()
                - (
                    current_allocation["roi"] * current_allocation["current_budget_pct"]
                ).sum(),
    ...
    def analyze_customer_acquisition_funnel(self):
        ...
        for stage in funnel_stages:
            stage_events = self.digital_events[
                self.digital_events["event_type"] == stage
            ]
            ...
            conversion_rate = (
                customers_who_purchased / unique_customers
            ...
            stage_customers = (
                stage_events.groupby("customer_id")["event_date"].min().reset_index()
            )
            ...
            conversion_times = stage_customers.merge(
                first_purchases, on="customer_id", how="inner"
            )
            ...
            avg_days_to_convert = (
                conversion_times["days_to_convert"].mean()
                if len(conversion_times) > 0
                else 0
            )
    ...
    def create_marketing_dashboard_data(self):
        ...
        dashboard_data = {
            "attribution_summary": {
                ...
                "total_attributed_revenue": channel_performance[
                    "attributed_revenue"
                ].sum(),
                "avg_customer_value": self.attribution_data[
                    "total_customer_value"
                ].mean(),
                "avg_touchpoints_per_customer": self.attribution_data[
                    "touchpoints_count"
                ].mean(),
                "top_performing_channel": channel_performance.iloc[0]["channel"],
                "best_roi_channel": channel_performance.loc[
                    channel_performance["roi"].idxmax(), "channel"
                ],
            },
...
def main():
    ...
    # Save results
    attribution_engine.attribution_data.to_csv(
        "results/marketing_attribution_data.csv", index=False
    )
