...
class BusinessInsightsEngine:
    ...
    def calculate_business_kpis(self):
        ...
        avg_items_per_order = (
            self.transactions.groupby("transaction_id")["quantity"].sum().mean()
        )
        ...
        if len(monthly_data) >= 2:
            revenue_growth = (
                (
                    monthly_data["total_amount"].iloc[-1]
                    - monthly_data["total_amount"].iloc[-2]
                )
                / monthly_data["total_amount"].iloc[-2]
                * 100
            )
            customer_growth = (
                (
                    monthly_data["customer_id"].iloc[-1]
                    - monthly_data["customer_id"].iloc[-2]
                )
                / monthly_data["customer_id"].iloc[-2]
                * 100
            )
    ...
    def detect_anomalies(self):
        ...
        daily_revenue = (
            self.transactions.groupby(self.transactions["transaction_date"].dt.date)[
                "total_amount"
            ]
            .sum()
            .reset_index()
        ...
        if len(daily_revenue) > 7:  # Need at least a week of data
            ...
            iso_forest = IsolationForest(
                contamination=self.thresholds["anomaly_sensitivity"], random_state=42
            )
        ...
        if len(customer_daily_counts) > 7:
            ...
            anomalous_customer_days = customer_daily_counts[
                customer_anomaly_labels == -1
            ]
        ...
        top_performers = product_sales[
            product_sales["total_amount"] > revenue_threshold
        ]

        for _, row in top_performers.iterrows():
            product_name = (
                self.products[self.products["product_id"] == row["product_id"]][
                    "product_name"
                ].iloc[0]
                if len(self.products[self.products["product_id"] == row["product_id"]])
                > 0
                else "Unknown"
            )
    ...
    def analyze_trends(self):
        ...
        if len(monthly_trends) >= 3:
            ...
            revenue_trend = np.polyfit(
                range(len(monthly_trends)), monthly_trends["revenue_total"], 1
            )[0]

            # Customer trend
            customer_trend = np.polyfit(
                range(len(monthly_trends)), monthly_trends["unique_customers"], 1
            )[0]

            # Transaction trend
            transaction_trend = np.polyfit(
                range(len(monthly_trends)), monthly_trends["transaction_count"], 1
            )[0]
            ...
            self.trends = {
                "revenue_trend": {
                    ...
                    "monthly_data": monthly_trends[["month", "revenue_total"]].to_dict(
                        "records"
                    ),
                },
                "customer_trend": {
    ...
    def generate_insights(self):
        ...
        high_severity_anomalies = [
            a for a in self.anomalies if a.get("severity") == "high"
        ]
    ...
    def calculate_business_health_score(self):
        ...
        anomaly_count = len(
            [a for a in self.anomalies if a.get("severity") in ["high", "medium"]]
        )
        if anomaly_count == 0:
    ...
    def generate_executive_summary(self):
        ...
        summary = {
            ...
            "top_insights": [
                insight for insight in self.insights if insight["impact"] == "high"
            ][:3],
            "critical_recommendations": [
                rec for rec in self.recommendations if rec["priority"] == "high"
            ],
            "anomalies_summary": {
