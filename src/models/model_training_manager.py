...
class ModelTrainingManager:
    ...
    def create_validation_curves(
        ...
        plt.plot(
            param_range, np.mean(train_scores, axis=1), "o-", label="Training Score"
        )
        plt.plot(
            param_range, np.mean(test_scores, axis=1), "o-", label="Validation Score"
        )
        plt.fill_between(
            param_range,
        ...
        # Save plot
        plot_path = (
            f"{self.results_dir}/model_performance/{model_name}_validation_curve.png"
        )
    ...
    def save_model_with_versioning(
        ...
        metadata = {
            ...
            "deployment_ready": not metrics.get("is_overfitting", False)
            and metrics.get("cv_mean", 0) > 0.7,
        }
        ...
        self.training_history.append(
            {
                ...
                "performance": metrics.get(
                    "test_auc" if "test_auc" in metrics else "test_r2", 0
                ),
