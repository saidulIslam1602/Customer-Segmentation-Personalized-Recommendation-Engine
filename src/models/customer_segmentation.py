...
class CustomerSegmentation:
    ...
    def calculate_customer_lifetime_value(self):
        ...
        clv_features['historical_clv'] = clv_features['monetary'] * clv_features['frequency']
        
        # Predictive features for CLV modeling
        clv_features['avg_days_between_purchases'] = clv_features['recency'] / clv_features['frequency']
        ...
        seasonal_data['month'] = pd.to_datetime(seasonal_data['transaction_date']).dt.month
        seasonal_data['quarter'] = pd.to_datetime(seasonal_data['transaction_date']).dt.quarter
        
        seasonal_patterns = seasonal_data.groupby(['customer_id', 'quarter']).agg({
            'total_amount': 'sum',
            'transaction_date': 'count'
        }).reset_index()
        
        seasonal_variance = seasonal_patterns.groupby('customer_id').agg({
            'total_amount': 'std',
            'transaction_date': 'std'
        }).fillna(0)
        
        clv_features = clv_features.merge(
            seasonal_variance.rename(columns={
        ...
        first_purchase = self.transactions.groupby('customer_id')['transaction_date'].min()
        last_purchase = self.transactions.groupby('customer_id')['transaction_date'].max()
        ...
        print(f"   CLV Range: ${clv_features['historical_clv'].min():.2f} - ${clv_features['historical_clv'].max():.2f}")
    ...
    def _train_clv_model(self, clv_features):
        ...
        feature_columns = [
            ...
            'purchase_frequency_per_month', 'avg_days_between_purchases',
            'purchase_acceleration', 'seasonal_spend_variance',
            'seasonal_frequency_variance', 'customer_tenure_days',
            'days_since_last_purchase'
        ]
        
        # Encode categorical features
        le = LabelEncoder()
        clv_features['lifecycle_stage_encoded'] = le.fit_transform(clv_features['lifecycle_stage'].astype(str))
    ...
    def create_behavioral_features(self):
        ...
        behavioral_data = self.transactions.groupby('customer_id').agg({
            'total_amount': ['sum', 'mean', 'std', 'min', 'max'],
            'quantity': ['sum', 'mean', 'std'],
            'transaction_date': ['count', 'min', 'max'],
            'product_id': 'nunique',
            'store_id': 'nunique',
            'channel': lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else 'Unknown',
            'payment_method': lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else 'Unknown'
        }).round(2)
        
        # Flatten column names
        behavioral_data.columns = ['_'.join(col).strip() for col in behavioral_data.columns]
        behavioral_data = behavioral_data.reset_index()
        
        # Time-based patterns
        transactions_with_time = self.transactions.copy()
        transactions_with_time['transaction_date'] = pd.to_datetime(transactions_with_time['transaction_date'])
        transactions_with_time['hour'] = transactions_with_time['transaction_date'].dt.hour
        ...
        time_patterns = transactions_with_time.groupby('customer_id').agg({
        ...
        time_patterns.columns = ['customer_id', 'preferred_hour', 'preferred_day', 'preferred_month']
        
        # Shopping consistency metrics
        consistency_metrics = transactions_with_time.groupby('customer_id').agg({
            'hour': 'std',
            'day_of_week': 'std',
            'total_amount': lambda x: x.std() / x.mean() if x.mean() > 0 else 0  # Coefficient of variation
        }).fillna(0).reset_index()
        
        consistency_metrics.columns = ['customer_id', 'hour_consistency', 'day_consistency', 'amount_consistency']
        
        # Purchase intervals
        purchase_intervals = transactions_with_time.groupby('customer_id')['transaction_date'].apply(
        ...
        behavioral_features = behavioral_data.merge(time_patterns, on='customer_id', how='left')
        behavioral_features = behavioral_features.merge(consistency_metrics, on='customer_id', how='left')
        behavioral_features = behavioral_features.merge(purchase_intervals, on='customer_id', how='left')
        
        # Product diversity metrics
        product_diversity = self.transactions.groupby('customer_id')['product_id'].apply(
            lambda x: len(set(x)) / len(x) if len(x) > 0 else 0  # Unique products ratio
        ).reset_index()
        product_diversity.columns = ['customer_id', 'product_diversity_ratio']
        
        behavioral_features = behavioral_features.merge(product_diversity, on='customer_id', how='left')
    ...
    def detect_vip_customers(self):
        ...
        vip_features = self.clv_data[[
            'monetary', 'frequency', 'historical_clv', 'predicted_clv',
        ...
        self.anomaly_detector = IsolationForest(
            contamination=0.05,  # Expect 5% VIPs
            random_state=42
        )
        
        anomaly_scores = self.anomaly_detector.fit_predict(vip_features_scaled)
        anomaly_scores_continuous = self.anomaly_detector.score_samples(vip_features_scaled)
        
        # VIP customers are those with high positive anomaly scores
        self.clv_data['vip_score'] = anomaly_scores_continuous
        self.clv_data['is_vip'] = (anomaly_scores == -1) & (self.clv_data['monetary'] > self.clv_data['monetary'].quantile(0.8))
        ...
        print(f"   📊 VIP contribution: {(vip_customers['historical_clv'].sum() / self.clv_data['historical_clv'].sum() * 100):.1f}% of total value")
    ...
    def create_segment_profiles(self):
        ...
        if self.clv_data is not None:
            profile_data = profile_data.merge(
                self.clv_data[['customer_id', 'historical_clv', 'predicted_clv', 'lifecycle_stage', 'is_vip']],
                on='customer_id', how='left'
            )
        
        if self.behavioral_features is not None:
            profile_data = profile_data.merge(self.behavioral_features, on='customer_id', how='left')
        ...
        for segment in segments:
            ...
            profile = {
                ...
                # CLV characteristics
                'avg_historical_clv': segment_data['historical_clv'].mean() if 'historical_clv' in segment_data.columns else 0,
                'avg_predicted_clv': segment_data['predicted_clv'].mean() if 'predicted_clv' in segment_data.columns else 0,
                'vip_percentage': segment_data['is_vip'].mean() * 100 if 'is_vip' in segment_data.columns else 0,
                
                # Behavioral characteristics
                'product_diversity': segment_data['product_diversity_ratio'].mean() if 'product_diversity_ratio' in segment_data.columns else 0,
                'purchase_consistency': segment_data['amount_consistency'].mean() if 'amount_consistency' in segment_data.columns else 0,
                
                # Business value
                'total_value': segment_data['monetary'].sum(),
                'value_contribution': segment_data['monetary'].sum() / profile_data['monetary'].sum() * 100
            }
    ...
    def generate_retention_strategies(self):
        ...
        for segment, profile in self.segment_profiles.items():
            ...
            if strategy['budget_allocation'] > 0:
                ...
                expected_revenue_increase = profile['total_value'] * expected_retention_rate
                strategy['expected_roi'] = (expected_revenue_increase / strategy['budget_allocation'] - 1) * 100
            
            self.retention_strategies[segment] = strategy
        
        print(f"✅ Retention strategies generated for {len(self.retention_strategies)} segments")
        
        # Print strategy summary
        total_budget = sum(s['budget_allocation'] for s in self.retention_strategies.values())
        weighted_roi = sum(s['expected_roi'] * s['budget_allocation'] for s in self.retention_strategies.values()) / total_budget if total_budget > 0 else 0
    ...
    def run_enhanced_segmentation(self):
        ...
        print(f"   👑 VIP Customers: {len(self.vip_customers) if self.vip_customers is not None else 0}")
    ...
    def save_enhanced_results(self):
        ...
        if self.rfm_data is not None:
            self.rfm_data.to_csv(f'reports/enhanced_segments_{timestamp}.csv', index=False)
        ...
        if self.vip_customers is not None:
            self.vip_customers.to_csv(f'reports/vip_customers_{timestamp}.csv', index=False)
        ...
        results_summary = {
            ...
            'total_vips': len(self.vip_customers) if self.vip_customers is not None else 0
        }
        ...
        if self.anomaly_detector is not None:
            joblib.dump(self.anomaly_detector, f'models/vip_detector_{timestamp}.joblib')
