...
class PricingOptimizationEngine:
    ...
    def prepare_pricing_data(self):
        ...
        self.transactions['year_week'] = self.transactions['transaction_date'].dt.strftime('%Y-%U')
        
        weekly_data = self.transactions.groupby(['product_id', 'year_week']).agg({
            'quantity': 'sum',
            'total_amount': 'sum',
        ...
        weekly_data['discount_percentage'] = (weekly_data['discount_amount'] / weekly_data['total_amount']).fillna(0)
        weekly_data['revenue'] = weekly_data['quantity'] * weekly_data['effective_price']
        weekly_data['units_per_transaction'] = weekly_data['quantity'] / weekly_data['transaction_count']
        
        # Time-based features
        weekly_data['transaction_date'] = pd.to_datetime(weekly_data['transaction_date'])
        ...
        weekly_data['price_change'] = weekly_data.groupby('product_id')['effective_price'].pct_change().fillna(0)
        weekly_data['price_trend_4w'] = weekly_data.groupby('product_id')['effective_price'].transform(
            lambda x: x.rolling(window=4, min_periods=1).apply(lambda y: np.polyfit(range(len(y)), y, 1)[0] if len(y) > 1 else 0)
        )
        
        # Competitive pricing features (simplified)
        category_avg_price = weekly_data.groupby(['category', 'year_week'])['effective_price'].mean().reset_index()
        category_avg_price.columns = ['category', 'year_week', 'category_avg_price']
        weekly_data = weekly_data.merge(category_avg_price, on=['category', 'year_week'], how='left')
        
        weekly_data['price_vs_category_avg'] = (weekly_data['effective_price'] / weekly_data['category_avg_price']) - 1
        weekly_data['is_premium_priced'] = (weekly_data['price_vs_category_avg'] > 0.1).astype(int)
        weekly_data['is_discount_priced'] = (weekly_data['price_vs_category_avg'] < -0.1).astype(int)
        
        # Calculate demand elasticity features
        for window in [4, 8, 12]:  # 4, 8, 12 weeks
            weekly_data[f'avg_quantity_{window}w'] = weekly_data.groupby('product_id')['quantity'].transform(
                lambda x: x.rolling(window=window, min_periods=1).mean()
            )
            weekly_data[f'avg_price_{window}w'] = weekly_data.groupby('product_id')['effective_price'].transform(
                lambda x: x.rolling(window=window, min_periods=1).mean()
            )
        
        # Product lifecycle and performance metrics
        product_performance = weekly_data.groupby('product_id').agg({
        ...
        product_performance.columns = [
            'total_quantity', 'avg_weekly_quantity', 'quantity_volatility',
            'total_revenue', 'avg_weekly_revenue',
            'avg_price', 'price_volatility', 'min_price', 'max_price',
            'total_transactions', 'first_sale_date', 'last_sale_date'
        ]
        ...
        product_performance['weeks_on_market'] = ((reference_date - product_performance['first_sale_date']).dt.days / 7).round(0)
        product_performance['price_range'] = product_performance['max_price'] - product_performance['min_price']
        product_performance['price_coefficient_variation'] = product_performance['price_volatility'] / product_performance['avg_price']
        
        # Merge performance metrics
        weekly_data = weekly_data.merge(product_performance[['product_id', 'weeks_on_market', 'price_range', 'price_coefficient_variation']], 
        ...
        print(f"📅 Time period: {weekly_data['transaction_date'].min().date()} to {weekly_data['transaction_date'].max().date()}")
    ...
    def calculate_price_elasticity(self):
        ...
        for product_id in self.pricing_data['product_id'].unique():
            product_data = self.pricing_data[self.pricing_data['product_id'] == product_id].copy()
            ...
            # Control variables
            control_vars = ['month', 'is_holiday_season', 'discount_percentage', 'price_vs_category_avg']
            ...
            try:
                ...
                if product_data['effective_price'].std() > 0:
                    price_corr = product_data['effective_price'].corr(product_data['quantity'])
                    
                    # Simple arc elasticity calculation
                    price_change_pct = (product_data['effective_price'].max() - product_data['effective_price'].min()) / product_data['effective_price'].mean()
                    quantity_change_pct = (product_data['quantity'].max() - product_data['quantity'].min()) / product_data['quantity'].mean()
                ...
                elasticity_results.append({
                    ...
                    'revenue_potential': 'High' if abs(price_elasticity) > 0.8 and mean_quantity > product_data['quantity'].median() else 'Medium' if abs(price_elasticity) > 0.5 else 'Low'
                })
                
            except Exception as e:
                print(f"   Warning: Could not calculate elasticity for product {product_id}: {str(e)}")
                continue
        ...
        if len(elasticity_df) > 0:
            ...
            print(f"   Elastic products: {len(elasticity_df[elasticity_df['elasticity_type'] == 'Elastic'])}")
            print(f"   Moderately Elastic: {len(elasticity_df[elasticity_df['elasticity_type'] == 'Moderately Elastic'])}")
            print(f"   Inelastic products: {len(elasticity_df[elasticity_df['elasticity_type'] == 'Inelastic'])}")
    ...
    def optimize_pricing_strategy(self, target_metric='revenue'):
        ...
        for _, product in elasticity_data.iterrows():
            ...
            if target_metric == 'revenue':
                ...
                if elasticity < -0.1:  # Elastic product
                    ...
                    suggested_price = max(current_price * 0.7, min(current_price * 1.3, suggested_price))
                    
                    price_change_pct = (suggested_price - current_price) / current_price
                    
                    # Estimate impact
                    estimated_quantity_change = elasticity * price_change_pct
                    estimated_new_quantity = current_quantity * (1 + estimated_quantity_change)
                    estimated_revenue_change = (suggested_price * estimated_new_quantity) / (current_price * current_quantity) - 1
            ...
            pricing_strategies.append({
                ...
                'current_weekly_revenue': product['total_revenue'] / product['weeks_analyzed'],
        ...
        pricing_strategies_df['priority_score'] = (
            ...
            (pricing_strategies_df['current_weekly_revenue'] / pricing_strategies_df['current_weekly_revenue'].max() * 0.3) +
            ((pricing_strategies_df['confidence'] == 'High').astype(int) * 0.3)
        )
        
        pricing_strategies_df = pricing_strategies_df.sort_values('priority_score', ascending=False)
        
        self.pricing_recommendations = pricing_strategies_df
        
        # Summary statistics
        total_potential_revenue_impact = (pricing_strategies_df['estimated_revenue_change'] * 
                                        pricing_strategies_df['current_weekly_revenue']).sum() * 52  # Annualized
        
        high_impact_products = len(pricing_strategies_df[pricing_strategies_df['estimated_revenue_change'].abs() > 0.1])
        
        print(f"✅ Pricing strategies generated for {len(pricing_strategies_df)} products")
        print(f"💰 Estimated annual revenue impact: ${total_potential_revenue_impact:,.0f}")
    ...
    def generate_promotional_pricing_analysis(self):
        ...
        for product_id in self.pricing_data['product_id'].unique():
            ...
            promotional_periods = product_data[product_data['discount_percentage'] > 0.05]  # >5% discount
            ...
            # Calculate lift
            quantity_lift = (promo_avg_quantity - regular_avg_quantity) / regular_avg_quantity if regular_avg_quantity > 0 else 0
            revenue_lift = (promo_avg_revenue - regular_avg_revenue) / regular_avg_revenue if regular_avg_revenue > 0 else 0
            
            # ROI calculation
            discount_cost = promo_avg_revenue * promo_avg_discount
            additional_revenue = promo_avg_revenue - regular_avg_revenue
            promo_roi = (additional_revenue - discount_cost) / discount_cost if discount_cost > 0 else 0
            ...
            promotional_analysis.append({
                ...
                'promo_effectiveness': 'High' if quantity_lift > 0.3 and promo_roi > 0 else 'Medium' if quantity_lift > 0.1 else 'Low',
        ...
        if len(promotional_df) > 0:
            ...
            print(f"📊 Average quantity lift: {promotional_df['quantity_lift'].mean():.1%}")
            print(f"💰 Average revenue lift: {promotional_df['revenue_lift'].mean():.1%}")
            print(f"📈 Average promotional ROI: {promotional_df['promotional_roi'].mean():.1%}")
    ...
    def create_pricing_dashboard_data(self):
        ...
        dashboard_data = {
            'pricing_summary': {
                'total_products_analyzed': len(elasticity_data) if elasticity_data is not None else 0,
                'elastic_products': len(elasticity_data[elasticity_data['elasticity_type'] == 'Elastic']) if elasticity_data is not None else 0,
                'inelastic_products': len(elasticity_data[elasticity_data['elasticity_type'] == 'Inelastic']) if elasticity_data is not None else 0,
                'high_revenue_potential': len(pricing_strategies[pricing_strategies['revenue_potential'] == 'High']) if pricing_strategies is not None else 0,
                'avg_price_elasticity': elasticity_data['price_elasticity'].mean() if elasticity_data is not None else 0
            },
            'top_pricing_opportunities': pricing_strategies.head(20).to_dict('records') if pricing_strategies is not None else [],
            'promotional_insights': {
                'avg_quantity_lift': promotional_analysis['quantity_lift'].mean() if promotional_analysis is not None else 0,
                'avg_revenue_lift': promotional_analysis['revenue_lift'].mean() if promotional_analysis is not None else 0,
                'high_effectiveness_promos': len(promotional_analysis[promotional_analysis['promo_effectiveness'] == 'High']) if promotional_analysis is not None else 0
            }
        }
...
def main():
    ...
    if pricing_strategies is not None:
        pricing_strategies.to_csv('results/pricing_optimization_strategies.csv', index=False)
    
    if promotional_analysis is not None:
        promotional_analysis.to_csv('results/promotional_pricing_analysis.csv', index=False)
