...
class FraudDetectionEngine:
    ...
    def prepare_fraud_features(self):
        ...
        transaction_features['day_of_week'] = transaction_features['transaction_date'].dt.dayofweek
        transaction_features['is_weekend'] = transaction_features['day_of_week'].isin([5, 6]).astype(int)
        transaction_features['is_night'] = transaction_features['hour'].between(22, 6).astype(int)
        transaction_features['is_business_hours'] = transaction_features['hour'].between(9, 17).astype(int)
        
        # Amount-based features
        transaction_features['amount_rounded'] = (transaction_features['total_amount'] % 1 == 0).astype(int)
        transaction_features['high_amount'] = (transaction_features['total_amount'] > transaction_features['total_amount'].quantile(0.95)).astype(int)
        transaction_features['low_amount'] = (transaction_features['total_amount'] < transaction_features['total_amount'].quantile(0.05)).astype(int)
        
        # Quantity-based features
        transaction_features['high_quantity'] = (transaction_features['quantity'] > transaction_features['quantity'].quantile(0.95)).astype(int)
        transaction_features['unusual_quantity'] = (transaction_features['quantity'] > 10).astype(int)
        
        # Discount-based features
        transaction_features['has_discount'] = transaction_features['discount_applied'].astype(int)
        transaction_features['high_discount'] = (transaction_features['discount_amount'] > transaction_features['discount_amount'].quantile(0.9)).astype(int)
        
        # Customer behavior aggregations
        customer_behavior = self.transactions.groupby('customer_id').agg({
        ...
        customer_behavior.columns = [
            ...
            'total_spent', 'avg_transaction_amount', 'amount_std', 'min_amount', 'max_amount',
            'total_items', 'avg_items_per_transaction', 'max_items_per_transaction',
            'unique_stores', 'unique_channels', 'unique_payment_methods', 'discount_transactions'
        ]
        customer_behavior = customer_behavior.reset_index()
        
        # Calculate behavioral metrics
        customer_behavior['days_active'] = (customer_behavior['last_transaction'] - customer_behavior['first_transaction']).dt.days + 1
        customer_behavior['transaction_frequency'] = customer_behavior['total_transactions'] / customer_behavior['days_active']
        customer_behavior['amount_volatility'] = customer_behavior['amount_std'] / (customer_behavior['avg_transaction_amount'] + 1)
        customer_behavior['discount_rate'] = customer_behavior['discount_transactions'] / customer_behavior['total_transactions']
        
        # Add customer demographics
        customer_behavior = customer_behavior.merge(self.customers, on='customer_id', how='left')
        
        # Calculate account age
        customer_behavior['account_age_days'] = (reference_date - customer_behavior['member_since']).dt.days
        customer_behavior['new_account'] = (customer_behavior['account_age_days'] < 30).astype(int)
        
        # Velocity features (recent activity)
        recent_date = reference_date - timedelta(days=7)
        recent_transactions = self.transactions[self.transactions['transaction_date'] >= recent_date]
        
        recent_activity = recent_transactions.groupby('customer_id').agg({
            'total_amount': ['sum', 'count'],
            'store_id': 'nunique',
            'transaction_date': lambda x: x.dt.date.nunique()
        }).round(2)
        
        recent_activity.columns = ['recent_7d_amount', 'recent_7d_transactions', 'recent_7d_stores', 'recent_7d_active_days']
        recent_activity = recent_activity.reset_index()
        
        # Merge recent activity
        customer_behavior = customer_behavior.merge(recent_activity, on='customer_id', how='left')
        customer_behavior[['recent_7d_amount', 'recent_7d_transactions', 'recent_7d_stores', 'recent_7d_active_days']] = customer_behavior[['recent_7d_amount', 'recent_7d_transactions', 'recent_7d_stores', 'recent_7d_active_days']].fillna(0)
        
        # Calculate velocity ratios
        customer_behavior['recent_vs_avg_amount'] = customer_behavior['recent_7d_amount'] / (customer_behavior['avg_transaction_amount'] * 7 + 1)
        customer_behavior['recent_vs_avg_frequency'] = customer_behavior['recent_7d_transactions'] / (customer_behavior['transaction_frequency'] * 7 + 1)
        
        # Geographic features
        customer_behavior['postal_code_first_digit'] = customer_behavior['postal_code'].astype(str).str[0]
        ...
        customer_behavior['gender_encoded'] = le_gender.fit_transform(customer_behavior['gender'].fillna('Unknown'))
        customer_behavior['loyalty_tier_encoded'] = le_loyalty.fit_transform(customer_behavior['loyalty_tier'].fillna('Bronze'))
        customer_behavior['language_encoded'] = le_language.fit_transform(customer_behavior['preferred_language'].fillna('NO'))
        customer_behavior['postal_first_digit_encoded'] = le_postal.fit_transform(customer_behavior['postal_code_first_digit'].fillna('0'))
        
        # Risk indicators
        customer_behavior['high_velocity'] = (customer_behavior['recent_vs_avg_frequency'] > 3).astype(int)
        customer_behavior['amount_spike'] = (customer_behavior['recent_vs_avg_amount'] > 5).astype(int)
        customer_behavior['multi_channel_recent'] = (customer_behavior['recent_7d_stores'] > 3).astype(int)
        customer_behavior['inconsistent_behavior'] = (customer_behavior['amount_volatility'] > 2).astype(int)
        ...
        fraud_indicators = (
            ...
            customer_behavior['new_account'] * (customer_behavior['recent_vs_avg_amount'] > 2).astype(int)
        )
        ...
        np.random.seed(42)
        additional_fraud = np.random.choice(customer_behavior.index, size=int(len(customer_behavior) * 0.02), replace=False)
    ...
    def train_fraud_detection_model(self):
        ...
        feature_columns = [
            'total_transactions', 'total_spent', 'avg_transaction_amount', 'amount_std',
            'total_items', 'avg_items_per_transaction', 'max_items_per_transaction',
            'unique_stores', 'unique_channels', 'unique_payment_methods',
            'transaction_frequency', 'amount_volatility', 'discount_rate',
            'account_age_days', 'age', 'household_size', 'estimated_income',
            'recent_7d_amount', 'recent_7d_transactions', 'recent_7d_stores',
            'recent_vs_avg_amount', 'recent_vs_avg_frequency',
            'gender_encoded', 'loyalty_tier_encoded', 'language_encoded',
            'high_velocity', 'amount_spike', 'multi_channel_recent', 'inconsistent_behavior'
        ]
    ...
    def calculate_risk_scores(self):
        ...
        anomaly_scores = self.fraud_model['isolation_forest'].decision_function(X_scaled)
        # Convert to 0-1 scale (higher = more anomalous)
        anomaly_scores_normalized = (anomaly_scores.max() - anomaly_scores) / (anomaly_scores.max() - anomaly_scores.min())
        
        # Combine scores
        combined_risk_score = (fraud_probabilities * 0.7) + (anomaly_scores_normalized * 0.3)
        
        # Create risk categories
        risk_categories = pd.cut(combined_risk_score, 
                               bins=[0, 0.3, 0.7, 1.0], 
        ...
        risk_results = risk_results.merge(
            self.fraud_features[['customer_id', 'total_spent', 'total_transactions', 'account_age_days']],
        ...
        print(f"   High Risk: {len(risk_results[risk_results['risk_category'] == 'High Risk'])}")
        print(f"   Medium Risk: {len(risk_results[risk_results['risk_category'] == 'Medium Risk'])}")
        print(f"   Low Risk: {len(risk_results[risk_results['risk_category'] == 'Low Risk'])}")
    ...
    def detect_transaction_anomalies(self):
        ...
        transaction_features = self.transactions.copy()
        transaction_features['transaction_date'] = pd.to_datetime(transaction_features['transaction_date'])
        ...
        transaction_features['amount_z_score'] = np.abs(
            (transaction_features['total_amount'] - transaction_features['total_amount'].mean()) / 
        ...
        customer_patterns = self.transactions.groupby('customer_id').agg({
            'total_amount': ['mean', 'std'],
            'quantity': ['mean', 'std'],
            'hour': lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else x.mean(),
            'store_id': lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else x.iloc[0]
        }).round(2)
        
        customer_patterns.columns = ['avg_amount', 'amount_std', 'avg_quantity', 'quantity_std', 'typical_hour', 'typical_store']
        customer_patterns = customer_patterns.reset_index()
        
        # Merge with transactions
        transaction_features = transaction_features.merge(customer_patterns, on='customer_id', how='left')
        ...
        transaction_features['hour_deviation'] = np.abs(transaction_features['hour'] - transaction_features['typical_hour'])
        transaction_features['different_store'] = (transaction_features['store_id'] != transaction_features['typical_store']).astype(int)
        ...
        anomalous_transactions = transaction_features[transaction_features['anomaly_score'] > 0.5].copy()
        ...
        print(f"📊 Anomaly rate: {len(anomalous_transactions) / len(transaction_features):.2%}")
        
        return anomalous_transactions[['transaction_id', 'customer_id', 'transaction_date', 'total_amount', 
                                     'quantity', 'anomaly_score', 'amount_deviation', 'quantity_deviation']]
    ...
    def generate_fraud_alerts(self):
        ...
        # High-risk customers
        high_risk_customers = risk_scores[risk_scores['risk_category'] == 'High Risk'].copy()
        high_risk_customers = high_risk_customers.sort_values('combined_risk_score', ascending=False)
        ...
        alerts = {
            ...
            'recent_anomalies': {
                ...
                'avg_anomaly_score': recent_anomalies['anomaly_score'].mean() if len(recent_anomalies) > 0 else 0
            },
            'recommendations': [
    ...
    def create_fraud_dashboard_data(self):
        ...
        dashboard_data = {
            'fraud_summary': {
                ...
                'high_risk_customers': len(risk_scores[risk_scores['risk_category'] == 'High Risk']),
                'medium_risk_customers': len(risk_scores[risk_scores['risk_category'] == 'Medium Risk']),
                'low_risk_customers': len(risk_scores[risk_scores['risk_category'] == 'Low Risk']),
