...
class RecommendationEngine:
    ...
    def _prepare_product_features(self):
        ...
        if 'subcategory' in products_features.columns:
            combined_features = combined_features + ' ' + products_features['subcategory'].fillna('')
        
        if 'brand' in products_features.columns:
            combined_features = combined_features + ' ' + products_features['brand'].fillna('')
            
        if 'organic' in products_features.columns:
            combined_features = combined_features + ' ' + products_features['organic'].astype(str)
            
        if 'local_producer' in products_features.columns:
            combined_features = combined_features + ' ' + products_features['local_producer'].astype(str)
    ...
    def get_collaborative_recommendations(self, customer_id, n_recommendations=10):
        ...
        for product_id, predicted_score in top_recommendations.items():
            ...
            result.append({
                ...
                'price': product_info.get('unit_price', product_info.get('price', 0)),
    ...
    def _train_hybrid_ml_model(self):
        ...
        if len(training_data) > 100:  # Ensure we have enough data
            # Convert to DataFrame
            feature_names = [
                'svd_score', 'nmf_score', 'content_score', 'popularity_score',
                'customer_age', 'customer_income', 'product_category_encoded',
                'recency_days', 'frequency_score', 'seasonal_factor'
            ]
    ...
    def _get_hybrid_features(self, customer_id, product_id, customer_data):
        ...
        try:
            ...
            customer_vector = self.collaborative_model.transform(self.user_item_matrix.iloc[[customer_idx]])
            product_vector = self.collaborative_model.components_[:, product_idx]
            svd_score = np.dot(customer_vector[0], product_vector)
            
            # NMF score
            customer_nmf = self.nmf_model.transform(self.user_item_matrix.iloc[[customer_idx]])
            ...
            return [
                svd_score, nmf_score, content_score, popularity_score,
                customer_age, customer_income, category_encoded,
                recency_days, frequency_score, seasonal_factor
            ]
        except:
            return None
    ...
    def _analyze_temporal_patterns(self):
        ...
        self.transactions['day_of_week'] = self.transactions['transaction_date'].dt.dayofweek
        self.transactions['hour'] = self.transactions['transaction_date'].dt.hour
        
        # Seasonal patterns by product category
        seasonal_data = self.transactions.merge(self.products[['product_id', 'category']], on='product_id')
        
        self.seasonal_patterns = seasonal_data.groupby(['category', 'month']).agg({
            'quantity': 'sum',
            'total_amount': 'sum'
        }).reset_index()
    ...
    def get_enhanced_recommendations(self, customer_id, n_recommendations=10):
        ...
        collab_recs = self.collaborative_recommendations(customer_id, n_recommendations * 2)
        content_recs = self.content_based_recommendations(customer_id, n_recommendations * 2)
        ...
        for product_id in list(all_products)[:50]:  # Limit for efficiency
            ...
            if features is not None:
                ...
                final_score = self._apply_business_rules(customer_id, product_id, hybrid_score)
        ...
        if self.business_rules['exclude_recently_purchased']:
            recent_purchases = self.transactions[
                ...
                (self.transactions['transaction_date'] >= 
                 datetime.now() - timedelta(days=self.business_rules['recency_days']))
            ]
    ...
    def save_enhanced_models(self):
        ...
        if self.collaborative_model is not None:
            joblib.dump(self.collaborative_model, f'models/svd_model_{timestamp}.joblib')
        ...
        config = {
            ...
            'timestamp': timestamp,
            'seasonal_patterns_shape': self.seasonal_patterns.shape if self.seasonal_patterns is not None else None
        }
