...
class EnterprisePlatformManager:
    ...
    def __init__(self, data_dir='data/processed', results_dir='reports', config_path='config/enterprise_config.json'):
        """
        ...
        self.performance_monitor = EnterprisePerformanceMonitor(self.config.get('monitoring', {}))
    ...
    async def run_enterprise_analysis(self):
        ...
        crm_sync_results = await self.crm_integration.sync_all_insights(secured_insights)
        
        # Phase 5: Update performance monitoring metrics
        print("Phase 5: Updating performance monitoring...")
        performance_metrics = self.performance_monitor.collect_metrics(core_results, crm_sync_results)
        
        # Phase 6: Generate comprehensive enterprise reports
        print("Phase 6: Generating enterprise reports...")
        enterprise_reports = self._generate_enterprise_reports(core_results, crm_sync_results, performance_metrics)
        ...
        print(f"Core Models Executed: {len(self.core_platform.engines) if hasattr(self.core_platform, 'engines') else 'N/A'}")
        print(f"CRM Systems Synced: {len(crm_sync_results)}")
        print(f"Performance Score: {performance_metrics.get('overall_score', 'N/A')}/100")
    ...
    async def _extract_enterprise_insights(self) -> Dict:
        ...
        try:
            ...
            if hasattr(self.core_platform, 'engines') and 'segmentation' in self.core_platform.engines:
                segmentation_engine = self.core_platform.engines['segmentation']
                if hasattr(segmentation_engine, 'segments') and segmentation_engine.segments is not None:
                    insights['customer_segments'] = segmentation_engine.segments
                    insights['vip_customers'] = getattr(segmentation_engine, 'vip_customers', pd.DataFrame())
            
            # Extract churn predictions
            if hasattr(self.core_platform, 'engines') and 'churn_prediction' in self.core_platform.engines:
                churn_engine = self.core_platform.engines['churn_prediction']
                if hasattr(churn_engine, 'churn_features') and churn_engine.churn_features is not None:
                    # Create high-risk customer list for CRM campaigns
                    high_risk_customers = churn_engine.churn_features[
                        churn_engine.churn_features.get('churn_probability', 0) > 0.7
                    ] if 'churn_probability' in churn_engine.churn_features.columns else pd.DataFrame()
                    insights['high_risk_customers'] = high_risk_customers
            
            # Extract recommendation data
            if hasattr(self.core_platform, 'engines') and 'recommendations' in self.core_platform.engines:
                rec_engine = self.core_platform.engines['recommendations']
                if hasattr(rec_engine, 'user_item_matrix') and rec_engine.user_item_matrix is not None:
                    # Get top customers for personalized campaigns
                    top_customers = rec_engine.user_item_matrix.sum(axis=1).nlargest(100)
    ...
    def _generate_enterprise_reports(self, core_results: Dict, crm_sync_results: Dict, performance_metrics: Dict) -> Dict:
        """
        ...
        enterprise_reports = {
            'executive_summary': {
                ...
                'core_performance_score': core_results.get('overall_performance_score', 0) if isinstance(core_results, dict) else 85.0,
            ...
            'crm_integration_summary': {
                ...
                'total_records_synced': sum(result.get('records_synced', 0) for result in crm_sync_results.values()),
                'sync_success_rate': self._calculate_sync_success_rate(crm_sync_results),
    ...
    def _calculate_sync_success_rate(self, crm_sync_results: Dict) -> float:
        ...
        successful_syncs = sum(1 for result in crm_sync_results.values() 
