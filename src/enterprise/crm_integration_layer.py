...
class CRMIntegrationLayer:
    ...
    def _initialize_connectors(self):
        ...
        for system in self.enabled_systems:
            if system == 'dynamics365':
                self.connectors[system] = Dynamics365Connector(self.config.get('dynamics365', {}))
            elif system == 'salesforce':
                self.connectors[system] = SalesforceConnector(self.config.get('salesforce', {}))
            elif system == 'hubspot':
                self.connectors[system] = HubSpotConnector(self.config.get('hubspot', {}))
    ...
    async def _sync_to_system(self, system_name: str, connector: Any, insights: Dict) -> CRMSyncResult:
        ...
        if 'customer_segments' in insights and not insights['customer_segments'].empty:
            segments_result = await connector.update_customer_segments(insights['customer_segments'])
            records_synced += segments_result.get('count', 0)
        
        # Sync high-risk customers for retention campaigns
        if 'high_risk_customers' in insights and not insights['high_risk_customers'].empty:
            churn_result = await connector.create_retention_campaigns(insights['high_risk_customers'])
        ...
        if 'top_customers' in insights and not insights['top_customers'].empty:
            engagement_result = await connector.update_engagement_scores(insights['top_customers'])
    ...
    async def create_retention_campaigns(self, high_risk_df: pd.DataFrame) -> Dict:
        ...
        for _, customer in high_risk_df.head(50).iterrows():  # Limit for demo
            campaign = {
                ...
                'recommended_action': self._get_retention_action(customer.get('churn_probability', 0)),
