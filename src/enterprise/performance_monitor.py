...
class EnterprisePerformanceMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        self._record_metric("cpu_usage_percent", cpu_percent, "percent", "system", timestamp)
        
        # Memory metrics
        memory = psutil.virtual_memory()
        self._record_metric("memory_usage_percent", memory.percent, "percent", "system", timestamp)
        self._record_metric("memory_available_gb", memory.available / (1024**3), "GB", "system", timestamp)
        
        # Disk metrics
        disk = psutil.disk_usage('/')
        disk_percent = (disk.used / disk.total) * 100
        self._record_metric("disk_usage_percent", disk_percent, "percent", "system", timestamp)
        
        # Network metrics (if available)
        try:
            network = psutil.net_io_counters()
            self._record_metric("network_bytes_sent", network.bytes_sent, "bytes", "network", timestamp)
            self._record_metric("network_bytes_recv", network.bytes_recv, "bytes", "network", timestamp)
        except:
            pass
    
    def _record_metric(self, name: str, value: float, unit: str, category: str, 
                      timestamp: datetime = None, threshold: float = None):
    ...
    def collect_metrics(self, core_results: Dict, crm_sync_results: Dict) -> Dict:
        ...
        overall_score = self._calculate_overall_score(business_metrics, crm_metrics, system_metrics, ml_metrics)
    ...
    def _extract_crm_metrics(self, crm_sync_results: Dict) -> Dict:
        ...
        successful_syncs = sum(
            1 for result in crm_sync_results.values()
            if (hasattr(result, 'status') and result.status == 'success') or
               (isinstance(result, dict) and result.get('status') == 'success')
        )
        
        success_rate = (successful_syncs / len(crm_sync_results)) * 100 if crm_sync_results else 0
    ...
    def _get_current_system_metrics(self) -> Dict:
        ...
        return {
            'cpu_usage_percent': self.current_metrics.get('cpu_usage_percent', PerformanceMetric(
                datetime.now(), 'cpu_usage_percent', 0, 'percent', 'system'
            )).value,
            'memory_usage_percent': self.current_metrics.get('memory_usage_percent', PerformanceMetric(
                datetime.now(), 'memory_usage_percent', 0, 'percent', 'system'
            )).value,
            'disk_usage_percent': self.current_metrics.get('disk_usage_percent', PerformanceMetric(
    ...
    def _calculate_overall_score(self, business_metrics: Dict, crm_metrics: Dict, 
                                system_metrics: Dict, ml_metrics: Dict) -> float:
        """Calculate overall performance score"""
        scores = []
        
        # Business performance (40% weight)
        business_score = min(100, (business_metrics.get('roi_percentage', 0) / 1000) * 100)
    ...
    def _save_performance_report(self, performance_summary: Dict):
        ...
        try:
            ...
            with open(f'reports/performance/performance_report_{timestamp}.json', 'w') as f:
    ...
    def add_custom_metric(self, name: str, value: float, unit: str, category: str = "custom"):
