...
class ExecutiveDashboard:
    ...
    def calculate_executive_kpis(self):
        ...
        current_transactions = transactions[transactions['transaction_date'] >= current_period_start]
        ...
        customer_growth = (current_customers - previous_customers) / previous_customers if previous_customers > 0 else 0
        
        # Transaction KPIs
        current_transactions_count = len(current_transactions)
        previous_transactions_count = len(previous_transactions)
        transaction_growth = (current_transactions_count - previous_transactions_count) / previous_transactions_count if previous_transactions_count > 0 else 0
        
        # Average Order Value
        current_aov = current_transactions['total_amount'].mean()
        previous_aov = previous_transactions['total_amount'].mean()
        aov_growth = (current_aov - previous_aov) / previous_aov if previous_aov > 0 else 0
        
        # Customer Lifetime Value (simplified)
        customer_clv = transactions.groupby('customer_id')['total_amount'].sum().mean()
        
        # Customer Acquisition Cost (estimated)
        new_customers_current = customers[customers['member_since'] >= current_period_start]
        estimated_marketing_spend = current_revenue * 0.05  # Assume 5% of revenue on marketing
        cac = estimated_marketing_spend / len(new_customers_current) if len(new_customers_current) > 0 else 0
        ...
        # Channel performance
        channel_revenue = current_transactions.groupby('channel')['total_amount'].sum().to_dict()
    ...
    def generate_business_insights(self):
        ...
        if not all([self.churn_engine, self.inventory_engine, self.pricing_engine, 
                   self.fraud_engine, self.marketing_engine]):
        ...
        try:
            ...
            insights['churn'] = {
                'high_risk_customers': churn_data['churn_summary']['high_risk_customers'],
                'churn_rate': churn_data['churn_summary']['churn_rate'],
                'retention_opportunity': churn_data['churn_summary']['high_risk_customers'] * 150,  # Avg customer value
                'status': 'critical' if churn_data['churn_summary']['churn_rate'] > 0.15 else 'warning' if churn_data['churn_summary']['churn_rate'] > 0.10 else 'good'
            }
        ...
        # Inventory insights
        try:
            inventory_recs = self.inventory_engine.generate_procurement_recommendations()
            insights['inventory'] = {
                'high_priority_reorders': inventory_recs['high_priority_reorders']['count'],
                'overstock_alerts': inventory_recs['overstock_alerts']['count'],
                'total_reorder_value': inventory_recs['high_priority_reorders']['total_value'],
                'status': 'critical' if inventory_recs['high_priority_reorders']['count'] > 50 else 'warning' if inventory_recs['high_priority_reorders']['count'] > 20 else 'good'
            }
        except Exception as e:
        ...
        try:
            ...
            insights['pricing'] = {
                'high_revenue_potential': pricing_data['pricing_summary']['high_revenue_potential'],
                'elastic_products': pricing_data['pricing_summary']['elastic_products'],
                'avg_elasticity': pricing_data['pricing_summary']['avg_price_elasticity'],
                'status': 'opportunity' if pricing_data['pricing_summary']['high_revenue_potential'] > 10 else 'good'
            }
        except Exception as e:
        ...
        try:
            ...
            insights['fraud'] = {
                'high_risk_customers': fraud_data['fraud_summary']['high_risk_customers'],
                'fraud_rate': fraud_data['fraud_summary']['fraud_rate'],
                'anomalous_transactions': fraud_data['fraud_summary']['total_anomalous_transactions'],
                'status': 'critical' if fraud_data['fraud_summary']['fraud_rate'] > 0.05 else 'warning' if fraud_data['fraud_summary']['fraud_rate'] > 0.02 else 'good'
            }
        except Exception as e:
        ...
        try:
            ...
            insights['marketing'] = {
                'top_channel': marketing_data['attribution_summary']['top_performing_channel'],
                'best_roi_channel': marketing_data['attribution_summary']['best_roi_channel'],
                'avg_customer_value': marketing_data['attribution_summary']['avg_customer_value'],
                'total_attributed_revenue': marketing_data['attribution_summary']['total_attributed_revenue'],
                'status': 'good'
            }
        except Exception as e:
    ...
    def create_executive_summary(self):
        ...
        if self.insights_data.get('inventory', {}).get('status') in ['critical', 'warning']:
            recommendations.append({
                'priority': 'Medium',
                'category': 'Inventory',
        ...
        executive_summary = {
            ...
            'critical_alerts': [rec for rec in recommendations if rec['priority'] == 'High'],
            'opportunities': [rec for rec in recommendations if rec['priority'] == 'Medium'],
    ...
    def _identify_achievements(self):
        ...
        if self.kpi_data['revenue']['growth'] > 0.1:
            achievements.append(f"Strong revenue growth: {self.kpi_data['revenue']['growth']:.1%}")
        
        if self.kpi_data['customers']['growth'] > 0.05:
            achievements.append(f"Customer base expansion: {self.kpi_data['customers']['growth']:.1%}")
        
        if self.kpi_data['aov']['growth'] > 0.05:
            achievements.append(f"Increased average order value: {self.kpi_data['aov']['growth']:.1%}")
        
        if self.kpi_data['customer_metrics']['ltv_cac_ratio'] > 3:
            achievements.append(f"Healthy LTV:CAC ratio: {self.kpi_data['customer_metrics']['ltv_cac_ratio']:.1f}:1")
...
def main():
    ...
    print("=" * 50)
    print(f"Overall Business Health: {executive_summary['overall_health']['status']} ({executive_summary['overall_health']['score']:.0f}%)")
    
    print(f"\n💰 Key Metrics:")
    print(f"   Revenue Growth: {executive_summary['key_metrics']['revenue']['growth']:.1%}")
    print(f"   Customer Growth: {executive_summary['key_metrics']['customers']['growth']:.1%}")
    ...
    for i, action in enumerate(executive_summary['next_actions'], 1):
        print(f"   {i}. {action['recommendation']} (Impact: {action['impact']}, Effort: {action['effort']})")
