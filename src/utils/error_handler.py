...
class ErrorMonitor:
    ...
    def record_error(
        ...
        # Log the error
        logger.error(f"[{severity}] Error in {context}: {error}", extra=error_info)
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == "OPEN":
            if time.time() - self.last_failure_time >= self.timeout:
                ...
                logger.info(
                    f"Circuit breaker {self.name}: Attempting to close (HALF_OPEN)"
                )
            else:
...
def circuit_breaker_protected(
    name: str, failure_threshold: int = 5, timeout: float = 60.0
):
    """
