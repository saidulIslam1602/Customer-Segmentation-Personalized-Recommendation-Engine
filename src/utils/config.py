...
class Config:
    ...
    def __init__(self):
        ...
        self.database = DatabaseConfig(
            url=self._get_env("DATABASE_URL", "postgresql://bi_user:password@localhost:5432/enterprise_bi"),
        ...
        self.security = SecurityConfig(
            jwt_secret_key=self._get_env("JWT_SECRET_KEY", "change-this-to-a-secure-random-key"),
            jwt_algorithm=self._get_env("JWT_ALGORITHM", "HS256"),
            jwt_expiry_hours=self._get_int("JWT_EXPIRY_HOURS", 24),
            encryption_key=self._get_env("ENCRYPTION_KEY", "change-this-to-a-secure-random-key")
        )
        ...
        self.crm = CRMConfig(
            ...
            hubspot_base_url=self._get_env("HUBSPOT_BASE_URL", "https://api.hubapi.com")
        )
        ...
        self.monitoring = MonitoringConfig(
            ...
            email_smtp_server=self._get_env("ALERT_EMAIL_SMTP_SERVER", "smtp.gmail.com"),
        ...
        self.features = {
            ...
            "inventory_optimization": self._get_bool("FEATURE_INVENTORY_OPTIMIZATION", True)
        }
        
        # External Services
        self.mlflow_tracking_uri = self._get_env("MLFLOW_TRACKING_URI", "http://localhost:5000")
    ...
    def validate(self):
        ...
        if self.crm.sync_enabled:
            if not any([
                ...
                errors.append("At least one CRM system must be configured when CRM_SYNC_ENABLED is true")
