...
class DataValidator:
    ...
    def validate_transactions(self, df: pd.DataFrame) -> ValidationResult:
        ...
        self._check_missing_values(df, critical_columns=['CustomerID', 'Quantity', 'UnitPrice'])
        ...
        self.metrics['unique_customers'] = df['CustomerID'].nunique() if 'CustomerID' in df.columns else 0
        self.metrics['date_range'] = self._get_date_range(df, 'InvoiceDate') if 'InvoiceDate' in df.columns else None
        self.metrics['total_revenue'] = (df['Quantity'] * df['UnitPrice']).sum() if all(col in df.columns for col in ['Quantity', 'UnitPrice']) else 0
    ...
    def validate_products(self, df: pd.DataFrame) -> ValidationResult:
        ...
        # Check for missing values
        self._check_missing_values(df, critical_columns=['StockCode', 'Description'] if 'StockCode' in df.columns else [])
        
        # Data quality metrics
        self.metrics['total_products'] = len(df)
        self.metrics['unique_products'] = df['StockCode'].nunique() if 'StockCode' in df.columns else 0
    ...
    def _check_required_columns(self, df: pd.DataFrame, required: List[str], dataset_name: str):
            ...
            if col in df.columns:
                if not pd.api.types.is_numeric_dtype(df[col]):
                    try:
                        ...
                        self.errors.append(f"Column '{col}' has invalid data type, expected numeric")
    ...
    def _check_missing_values(self, df: pd.DataFrame, critical_columns: List[str]):
        ...
        for col in critical_columns:
            if col in df.columns:
                ...
                if missing_count > 0:
                    if missing_pct > 5:
                        self.errors.append(f"Column '{col}' has {missing_pct:.1f}% missing values ({missing_count} rows)")
                    else:
                        self.warnings.append(f"Column '{col}' has {missing_count} missing values ({missing_pct:.2f}%)")
    
    def _check_positive_values(self, df: pd.DataFrame, column: str, allow_zero: bool = False):
        ...
        if column in df.columns:
            ...
            if invalid_count > 0:
                self.errors.append(f"Column '{column}' has {invalid_count} non-positive values")
    ...
    def _validate_dates(self, df: pd.DataFrame, column: str):
        ...
        if column in df.columns:
            try:
                ...
                if invalid_dates > 0:
                    self.errors.append(f"Column '{column}' has {invalid_dates} invalid dates")
                
                # Check for future dates
                if not dates.isna().all():
                    max_date = dates.max()
                    if max_date > pd.Timestamp.now():
                        self.warnings.append(f"Column '{column}' contains future dates (max: {max_date})")
            except Exception as e:
                self.errors.append(f"Failed to parse dates in column '{column}': {str(e)}")
    ...
    def _check_duplicates(self, df: pd.DataFrame, subset: List[str]):
        ...
        if valid_subset:
            ...
            if duplicates > 0:
                ...
                if dup_pct > 1:
                    self.warnings.append(f"Found {duplicates} duplicate rows ({dup_pct:.1f}%)")
    ...
    def _detect_outliers(self, df: pd.DataFrame, column: str, dataset_name: str):
        ...
        if column in df.columns and pd.api.types.is_numeric_dtype(df[column]):
            ...
            if outliers > 0:
                ...
                self.warnings.append(f"Column '{column}' in {dataset_name} has {outliers} outliers ({outlier_pct:.1f}%)")
    
    def _get_date_range(self, df: pd.DataFrame, column: str) -> Optional[Dict[str, str]]:
    ...
    for filename, validation_func in files_to_validate.items():
        ...
        try:
            ...
            if result.is_valid:
                ...
                logger.warning(f"Validation failed for {filename}: {len(result.errors)} errors")
...
def generate_validation_report(results: Dict[str, ValidationResult]) -> str:
    ...
    report_lines.append(f"Overall Status: {'PASSED' if total_errors == 0 else 'FAILED'}")
